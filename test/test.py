...
def test_pil_helpers(deck):
    ...
    test_key_image_pil = NativeImageHelper.create_key_image(deck)
    test_scaled_key_image_pil = NativeImageHelper.create_scaled_key_image(deck, test_key_image_pil)     # noqa: F841
    test_key_image_native = NativeImageHelper.to_native_key_format(deck, test_scaled_key_image_pil)     # noqa: F841

    if deck.is_touch():
        test_touchscreen_image_pil = NativeImageHelper.create_touchscreen_image(deck)
        test_scaled_touchscreen_image_pil = NativeImageHelper.create_scaled_touchscreen_image(deck, test_touchscreen_image_pil)     # noqa: F841
        test_touchscreen_image_native = NativeImageHelper.to_native_touchscreen_format(deck, test_scaled_touchscreen_image_pil)     # noqa: F841
...
def test_basic_apis(deck):
    with deck:
        ...
        if deck.is_visual():
            ...
            test_key_image_pil = NativeImageHelper.create_key_image(deck)
            test_key_image_native = NativeImageHelper.to_native_key_format(deck, test_key_image_pil)
            deck.set_key_image(0, None)
            deck.set_key_image(0, test_key_image_native)

            if deck.is_touch():
                test_touchscreen_image_pil = NativeImageHelper.create_touchscreen_image(deck)
                test_touchscreen_image_native = NativeImageHelper.to_native_touchscreen_format(deck, test_touchscreen_image_pil)
...
def test_key_pattern(deck):
    ...
    test_key_image = NativeImageHelper.create_key_image(deck)

    draw = ImageDraw.Draw(test_key_image)
    draw.rectangle((0, 0) + test_key_image.size, fill=(0x11, 0x22, 0x33), outline=(0x44, 0x55, 0x66))

    test_key_image = NativeImageHelper.to_native_key_format(deck, test_key_image)
